...
def entry_to_correct_format(UserInput):
    '''
...
def main():
    ...
    while Jump < JumpLimit :
        ...
        if raw_response is None or "error" in raw_response.lower():
            better_print("Pas de reponse ou erreur lors de l execution de la commande CLI: " + str(raw_response))
            return
        ...
        if GraphQL_Response:
            ...
            if switch_ip==None:
                better_print("Impossible de recuperer l'IP du switch " + str(tunnel_info) + " depuis la reponse GraphQL")
                return
            ...
            better_print("La reponse GraphQL est vide")
            return
    ...
    nouvelle_reponse_brute = execute_cli_command(NewCommand)
